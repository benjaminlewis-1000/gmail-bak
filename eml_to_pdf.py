...
# Source : https://stackoverflow.com/questions/31392361/how-to-read-eml-file-in-python/31392574
...
def pullout (m, key):
    ...
    if not m.is_multipart():
        ...
        if cp=="text/plain": 
            data = m.get_payload(decode=True).decode('utf-8')
            # print(data)
            Text += data
        elif cp=="text/html": 
            data = m.get_payload(decode=True).decode('utf-8')
...
def caption (origin):
    ...
    """
    Date = ""
    if 'Date' in origin.keys(): # origin.has_key("date"): 
        Date = origin["Date"].strip()
    From = ""
    if 'From' in origin.keys(): # origin.has_key("from"): 
        From = origin["from"].strip()
    To = ""
    if 'To' in origin.keys(): # origin.has_key("to"): 
        To = origin["to"].strip()
    Subject = ""
    if 'Subject' in origin.keys(): # origin.has_key("subject"): 
...
for root, dirs, files in os.walk(path):
    for f in files:
        ...
        if file.endswith('.eml'):
            handle = open(file)
            data = extract(handle, file)
            for k in data.keys():
                print(k, data[k])
        
f = open(os.path.join(path, "2018_August_31-23_08_26.eml"))
